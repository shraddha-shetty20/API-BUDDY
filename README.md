...
A lightweight REST API testing extension for Visual Studio Code.
...
flowchart TD
    ...
    A[VS Code User] --> B[API Buddy UI<br/>ui.html]

    B -->|Send Request| C[VS Code Extension<br/>extension.js]

    C -->|HTTP Request| D[REST API / Server]

    D -->|HTTP Response| C

    C -->|Response Data| B

    B -->|Display| E[Status + Headers + Response]

    F[package.json] -->|Extension Configuration| C
🔄 How It Works
The user enters the API URL, method, headers, and request body in the API Buddy UI.
ui.html sends the request details to extension.js.
extension.js sends the HTTP request to the target REST API.
The API returns the response.
extension.js sends the response back to the UI.
The UI displays the status, response time, headers, and response body.
📁 Project Structure
api-buddy/
│
├── extension.js
...
Response headers
Response body
REST API testing directly inside VS Code

### Important ⚠️

The architecture diagram **will not necessarily render inside the VS Code Markdown preview exactly like GitHub does**.
...
For the final version, GitHub will render the Mermaid diagram when you push the README.

One thing

Since we're building your project step-by-step, don't commit or push yet.

First open the Markdown preview and check whether the diagram looks good.

If it looks good, tell me WORKING, and we'll do the next README section. 🚀

full readme 

Absolutely. Since your current project has extension.js, ui.html, package.json, and .gitignore, here is a complete professional README you can paste into README.md.
...
├── ui.html            # API Buddy user interface
...
code .
▶️ Run the Extension in VS Code
Open the project folder in Visual Studio Code.
Open extension.js.
Press:
F5
VS Code will open a new Extension Development Host window.
Open API Buddy from the Command Palette.

You can open the Command Palette using:
...
API Buddy allows users to provide custom headers.
...
⚠️ Do not commit real API keys, passwords, access tokens, or other secrets to GitHub.

Use environment variables or another secure secret-management approach for sensitive credentials.

📊 Response Information

API Buddy displays important response information.
...
The API response is displayed in the response panel.

🧩 VS Code Extension Architecture

API Buddy uses the VS Code extension architecture:
...
Built with ❤️ using JavaScript and Visual Studio Code


### One important correction

Because your actual repository is **`API-BUDDY`**, the README above uses that exact repository name.

Now:

1. **Ctrl + A** inside `README.md`
2. Paste the complete content above.
3. **Ctrl + S**
4. Press **Ctrl + Shift + V** to preview it.

Don't commit or push yet. **First check the README preview**, especially the architecture diagram.
full document so i can paste

Absolutely 👍 Here is the full README.md document in one block. Just copy everything inside the code block and paste it into your README.md.
...
Check that the README looks good, especially the Architecture section